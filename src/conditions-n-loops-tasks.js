...
function convertToRomanNumerals(num) {
  let newNum = num;
  if (newNum < 1 || newNum > 39) {
    throw new Error('Input number is out of range (1-39)');
  }
...
function getBalanceIndex(arr) {
  ...
  let leftSum = 0;
  for (let i = 0; i < copyArr.length; i += 1) {
    const rightSum = totalSum - leftSum - copyArr[i];
    if (leftSum === rightSum) {
      return i;
    }
    leftSum += copyArr[i];
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < size; i += 1) {
    for (let j = 0; j < Math.floor(size / 2); j += 1) {
      const temp = rotatedMatrix[i][j];
      rotatedMatrix[i][j] = rotatedMatrix[i][size - j - 1];
      rotatedMatrix[i][size - j - 1] = temp;
    }
  }
